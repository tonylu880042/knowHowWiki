...
- 啟動掃描前，務必先使用 Terminal 讀取並檢查工作區內是否具備基礎目錄結構（包含 `raw/`、`raw/daily-logs/`、`processed/`、`wiki/` 等）。
- **如果發現目錄不存在或內容完全空白**：
  1. 請主動以指令建立這些遺失的目錄。
  2. 如果 `raw/` 是剛剛才建立或完全無檔案，請**暫停執行後續歸檔步驟**，並友善地回覆**引導使用者將欲處理的資料（PDF/對話/圖片等）放入 `raw/` 目錄**。待使用者放置完畢並再次呼叫你時，再繼續執行。
...
- **外部資料 (文字/網址/圖片/複雜文件)**：
    ...
    - **圖片與複雜文件 (PDF/PPT/Excel 等)**：
      ...
      3. **【內置 PDF 解析工具】**：針對 PDF 檔案，本 Skill 內建整合 Python 解析腳本。請使用 Terminal 執行 `python3 .agents/skills/llm-wiki-manager/scripts/pdf_parser.py <檔案或資料夾路徑>` 進行文字萃取（若提示缺乏 PyMuPDF，請先幫使用者代為執行 `pip install PyMuPDF`）。
      4. **【原生能力備案 (Fallback)】**：對於其他無法讀取的複雜格式或圖片，請降級使用你的「原生多模態視覺能力」或「內建檔案讀取工具 (File System)」親自打開讀取。
      5. 將最終獲得的核心重點、數據解譯成 Markdown 格式的解析短文，存入 `raw/`，格式：`YYYYMMDD-關鍵字-解析.md`，並在文中註記原始檔案名稱。
...
- **維護 DuckDB**：在 `processed/` 目錄下維護一個名為 `wiki_index.duckdb` 的本地資料庫。
- **資料更新 (Upsert)**：當任何 `wiki/` 目錄下的 Markdown 檔案被新增或更新時，請調用你的檔案執行工具（例如執行 Python 腳本），將該檔案的「檔名」、「相對路徑」、「標題」與「全文內容」同步更新至 DuckDB 的全文檢索資料表 (FTS Table) 中。
- **確保一致性**：DuckDB 內的資料必須與 `wiki/` 實體檔案保持 100% 同步。
