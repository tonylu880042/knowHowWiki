...
> **【為什麼選 DuckDB？】** 知識庫檢索層刻意選擇 **DuckDB + FTS**，而不是雲端 RAG (NotebookLM/Pinecone) 或 embedding vector store (pgvector)。關鍵優勢：
> - 🔒 **隱私 100% 留在本機**，敏感筆記不離開硬碟
> - 💰 **零成本、零依賴**，MIT 授權、無 server、無 API 額度
> - ⚡ **重建索引秒級完成**，所以 `wiki_index.duckdb` 可放進 `.gitignore`，新機器 clone 後重跑一次就復原
> - 🔍 **原生 FTS 中英文混合檢索**，不需要 embedding model
> - 🧮 **完整 SQL 可任意組合**，未來加過濾條件無痛擴充
> - 📦 **單一檔案 portable**，跨作業系統複製即遷移
> - 🪶 對 <1000 份文件的個人知識庫，FTS 比 vector search 更準更輕
>
> 因此**這一步是整個 skill 不可省略的核心環節**，每次 wiki/ 有變動都必須重建索引。
...
- **不要**自己撰寫 DuckDB SQL，一律使用此腳本（如需查詢交由 `llm-wiki-retriever` skill 處理）。
