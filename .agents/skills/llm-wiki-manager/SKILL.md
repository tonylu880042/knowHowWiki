...
- **大型 `.md` 檔案（>5KB）讀取規則**：禁止讀取全文。**只能用 shell 取前 100 行**：
  ```bash
  head -n 100 "raw/<檔名>.md"
  ```
  從前 100 行提煉 **3–5 個核心主題**，禁止嘗試重新生成或展開原文。
...
> 1. 使用者在訊息中**明確**使用「記錄 session / 整理本次對話 / 做成 daily log / 提煉決策與教訓」等關鍵字。
> 2. `raw/daily-logs/` 目錄中**已經存在**使用者預先放入的對話逐字稿檔案，需要提煉。
>
> **【嚴禁腦補】** 不要因為「現在正在跟使用者對話」就自動把當前 conversation 歷史提煉成 daily log。使用者呼叫 skill 掃描 raw/ 的意圖是處理 raw/ 中的檔案，**不是**記錄本次對話。若不確定，**直接跳過 1D**。

**啟動條件滿足時的執行規則：**
- 輸出檔案：`raw/daily-logs/YYYYMMDD-session-<關鍵字>.md`
- 必須包含三個區塊：`## 1. 討論脈絡`、`## 2. 做出的決策`、`## 3. 學到的教訓/陷阱`
- **【篇幅紅線】** 整份文件 **≤ 200 行 / ≤ 8KB**。每個區塊最多 5 個 bullet，每個 bullet ≤ 2 行。禁止貼逐字稿、禁止貼大段 code diff、禁止貼工具呼叫原始輸出。只留「決策與教訓」的精煉陳述。
- **【寫入方式紅線】** 先用 Write 工具建立一個包含三個區塊標題的骨架檔案（約 20 行），**立刻送出**；然後再用 Edit 逐段填入內容。**禁止在單次 Write 呼叫中塞入超過 200 行的 content 參數**，否則 Write 會卡在「Creating +0 -0 🔄」。
...
- **【大型檔案讀取紅線】處理 >5KB 的 .md 檔時，禁止用任何工具讀取全文**。只能用 `head -n 100` 取前 100 行提煉核心主題。
